...
import { formatDateLabel, formatMonthHeading } from "../utils/data.js";

export function createTestList({ tests, selectedMonth }) {
  const card = createElement("section", { className: "list-card" }, [
    createElement("div", { className: "subsection-head" }, [
      ...
      createElement("p", {
        ...
        text: `${formatMonthHeading(selectedMonth)} 内部滚动浏览`,
      }),
    ]),
  ]);
  ...
  if (!tests.length) {
    card.append(
      createElement("div", { className: "empty-card" }, [
        createElement("p", { text: "当前月份暂无可用性测试记录。" }),
      ]),
    );
    return card;
  }

...
function createHeader(state) {
  ...
  return createElement("header", { className: "editor-header" }, [
    createElement("div", { className: "editor-header-copy" }, [
      ...
      createElement("h1", { text: "月度成果看板内容录入页" }),
      createElement("p", {
        className: "editor-lead",
        text: "这里用于本地维护 A 区测试记录和 B 区洞察卡片。不会出现在公开看板导航里，适合你自己整理月份内容后再导出 JSON。",
      }),
    ]),
    createElement("div", { className: "editor-status-grid" }, [
      createStatCard("A 区记录", `${counts.usability} 条`),
      createStatCard("B 区卡片", `${counts.insights} 条`),
...
function createToolbar(state, root) {
  ...
  return createElement("section", { className: "editor-toolbar" }, [
    ...
    createElement("div", { className: "editor-toolbar-row editor-toolbar-row-bottom" }, [
      createElement("div", { className: "editor-month-filter" }, [
        createElement("span", { className: "editor-filter-label", text: "按月份查看" }),
        createElement(
          "div",
          { className: "editor-month-tabs" },
          [
...
function createRecordListItem(record, state, root) {
  ...
  const subline =
    activeTab === "usability"
      ? `${record.testDate || ""}${record.projectName ? ` · ${record.projectName}` : ""}`
      : `${record.month || ""}${record.featured ? " · 已入选" : " · 未入选"}`;
...
function createUsabilityEditor(record, state, root) {
  const shell = createElement("div", { className: "editor-panel" }, [
    createElement("div", { className: "editor-panel-head" }, [
      createElement("div", {}, [
        createElement("p", { className: "editor-muted", text: "A 区可用性测试记录" }),
  ...
  shell.append(
    createSectionBlock("基础信息", [
      ...
      createFieldGrid(
        createNumberField("本月累计发现问题", record.issueCount, (value) => updateRecordField(state, "usability", record.id, "issueCount", value), root),
...
function createInsightEditor(record, state, root) {
  const shell = createElement("div", { className: "editor-panel" }, [
    createElement("div", { className: "editor-panel-head" }, [
      createElement("div", {}, [
        createElement("p", { className: "editor-muted", text: "B 区研究洞察卡片" }),
        createElement("h2", { text: record.title || "未命名洞察" }),
      ]),
      createElement("div", { className: "editor-badge-group" }, [
        createElement("span", { className: "editor-badge", text: record.month }),
        createElement("span", {
          className: `editor-badge ${record.featured ? "editor-badge-soft" : ""}`,
          text: record.featured ? "当月展示" : "未入选",
        }),
      ]),
    ]),
  ]);
  ...
  shell.append(
    createSectionBlock("基础信息", [
      createFieldGrid(
        ...
        createMonthField("归属月份", record.month, (value) => {
        ...
        createToggleField("是否入选当月展示", record.featured, (value) => updateRecordField(state, "insights", record.id, "featured", value), root),
      ),
      createFieldGrid(
    ...
    createElement("div", { className: "editor-preview-head" }, [
      createElement("div", { className: "editor-preview-main" }, [
        ...
        createElement("p", {
          ...
          text: [record.testDate || "未填写日期", record.projectName || "未填写项目名"]
            .filter(Boolean)
            .join(" · "),
        }),
      ]),
...
function createToggleField(label, value, onCommit, root) {
  ...
  return createInputShell(label, createElement("label", { className: "editor-checkbox-row" }, [
    input,
    createElement("span", { text: value ? "已入选" : "未入选" }),
  ]));
}
...
function validateInsightRecord(record) {
  ...
  if (!record.featured) {
    tips.push("当前卡片未入选当月展示，公开看板中不会显示。");
  }
